...
// Store selected month per company
let companyMonthState = {};
...
function renderAllTables(selectedCompanies) {
    ...
    selectedCompanies.forEach(company => {
        if (!companyMonthState[company]) {
            companyMonthState[company] = new Date();
        }
        let monthDate = companyMonthState[company];
        let mKey = monthKey(monthDate);
        ...
        let html = `
            ...
            <div class="month-nav">
                <span class="month-arrow" onclick="changeMonth('${company}', -1)">←</span>
                <span class="text-dark">${formatMonthLabel(monthDate)}</span>
                <span class="month-arrow" onclick="changeMonth('${company}', 1)">→</span>
            </div>
        `;
        ...
        monthData.forEach(row => {
            ...
            let startDate = new Date(monthDate.getFullYear(), monthDate.getMonth(), 1);
...
function changeMonth(company, dir) {
    companyMonthState[company] = shiftMonth(companyMonthState[company], dir);
    renderAllTables(Object.keys(companyMonthState));
}

document.getElementById('company').addEventListener('change', function() {
    if (this.value === 'Demo') {
        renderAllTables(Object.keys(monthlyCompanyData));
    } else if (monthlyCompanyData[this.value]) {
        renderAllTables([this.value]);
    } else {
        document.getElementById('companyDetails').innerHTML = '';
        document.getElementById('grandTotalTable').innerHTML = '';
        document.getElementById('generateInvoiceBtnContainer').style.display = 'none';
    }
});
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u527856812/domains/dodgerblue-lapwing-476569.hostingersite.com/public_html/resources/views/property/selectProperty.blade.php ENDPATH**/ ?>