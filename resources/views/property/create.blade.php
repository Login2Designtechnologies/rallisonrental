...
<style>
  ...
  padding-top: 15px;
    ...
    .step-status2.done::before {
        content: "✔";  /* green tick */
        color: #28a745;
        font-weight: bold;font-size: 20px;
    }
    .step-status2.done{position: absolute;
  right: 10px;}
  .row.position-relative.mb-2.input-row ~ .row.position-relative.mb-2.input-row {
  border-top: 1px solid #ddd;
  padding-top: 15px;
...
@push('script-page')
    ...
    <script>
        $(document).ready(function () {
            ...
            /*// Destroy any existing Dropzone instance if re-initializing
            if (Dropzone.instances.length > 0) {
                Dropzone.instances.forEach(dz => dz.destroy());
            }

            // ✅ Working Dropzone instance
            var dropzone = new Dropzone("#demo-upload", {
                url: "#", // Dummy URL (we're not auto-uploading)
            ...
            // Stop Dropzone auto init
            if (typeof Dropzone !== 'undefined') Dropzone.autoDiscover = false;

            // Select Next button for this tab
            var nextBtn = $('.nextButton22[data-next-tab="#profile-2"]');
            nextBtn.prop('disabled', true).addClass('disabled');

            // Create a hidden input (if not already exists)
            ...
            // Function to check and enable/disable next button
            function updateNextBtnState() {
                var hiddenVal = $('#uploaded_image').val().trim();
                if (hiddenVal !== '') {
            ...
            // ✅ If there are existing images, mark hidden value + enable Next
            if ($('.existing-images img').length > 0) {
                $('#uploaded_image').val('existing');
                updateNextBtnState();
            }

            // ✅ If an existing image is deleted manually
            $(document).on('click', '.remove-image-btn', function () {
                setTimeout(function () {
                    if ($('.existing-images img').length === 0 && dropzone.files.length === 0) {
                        $('#uploaded_image').val('');
                    }
                    updateNextBtnState();
                }, 300);
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateNextButton() {
                ...
                if (!allFilled) {
                    $('a[href="#' + currentTabId + '"] .step-status2')
                        .html('')
                        .removeClass('done');
                }
            }
            ...
            $(document).on('click', '.nextButton', function() {
                ...
                let currentTabId = $activeTab.attr('id');
                $('a[href="#' + currentTabId + '"]').find('.step-status2').addClass('done');
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.save-unit', function() {
                ...
                toastrs('success', 'Unit saved successfully.', 'Success');
                 // ✅ Add "done" class to step indicator
                const stepIndicator = $('#profile-tab-3').find('.unitdata');
            ...
            $(document).on('click', '.remove-unit', function() {
                ...
                // ✅ If no more saved units exist, remove "done" class
                if ($('.unit_list').length === 0) {
                    $('#profile-tab-3').find('.unitdata').removeClass('done');
                }
                    ...
                    <ul class="nav nav-tabs profile-tabs" id="myTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="profile-tab-1" data-bs-toggle="tab" href="#profile-1"
                                ...
                                {{ __('Property Details') }}
                            @if(!empty($property->type) && !empty($property->name) && !empty($property->address) && !empty($property->state_id) && !empty($property->city_id) && !empty($property->zip_code))
                                <span class="step-status2 done"></span>
                            @else
                                <span class="step-status2"></span>
                            @endif
                            </a>
                        </li>
                        <li class="nav-item">
                        ...
                        <li class="nav-item">
                            ...
                            @if(isset($units) && $units->isNotEmpty())
                                <span class="step-status unitdata done"></span>
                            @else
                                <span class="step-status unitdata"></span>
                            @endif
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" id="profile-tab-4" data-bs-toggle="tab" href="#profile-4" role="tab"
                                ...
                                @if(isset($amenities) && $amenities->isNotEmpty())
                                   <span class="step-status amenitiesdata done"></span>
                                @else
                                   <span class="step-status amenitiesdata"></span>
                                @endif
                            </a>
                        </li>
                        <li class="nav-item">
                            ...
                            @php
                                ...
                                $groupedUtilities = $utilities
                                    ...
                                    ->values();
                            @endphp
                            @if(isset($groupedUtilities) && $groupedUtilities->isNotEmpty())
                                <span class="step-status utilitiesdata done"></span>
                            @else
                                <span class="step-status utilitiesdata"></span>
                            @endif
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" id="profile-tab-6" data-bs-toggle="tab" href="#profile-6" role="tab"
                                ...
                                <i class="material-icons-two-tone ti ti-bulb me-2"></i>
                                {{ __('Our Cost') }}
                            @if(!empty($property->mortgage_amount) && !empty($property->insurance_amount) && !empty($property->amenities_amount))
                    ...
                    <div class="tab-pane" id="profile-2" role="tabpanel" aria-labelledby="profile-tab-2">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card border bg-custom bg-white mb-2">
                                    <div class="card-header">
                                        {{ Form::label('demo-upload', __('Add Property Images'), ['class' => 'form-label']) }} <span class="text-danger">*</span>
                                    </div>
                                    <div class="card-body w-100">
                                        <div class="row">
                    ...
                    <div class="tab-pane" id="profile-4" role="tabpanel" aria-labelledby="profile-tab-4">
                      <div class="card bg-white p-3">
                        <div class="form-group mb-3">
                          <label>Do we have amenities in the property to be billed out?</label><br>
                          <label class="me-3">
                            <input type="radio" name="is_billed" value="yes" class="form-check-input"
                              {{ $hasAmenities ? 'checked' : '' }}> Yes
                          </label>
                          <label>
                            <input type="radio" name="is_billed" value="no" class="form-check-input"
                              {{ !$hasAmenities ? 'checked' : '' }}> No
                          </label>
                        </div>

                        <!-- Add Amenity Button -->
                        <div id="addAmenityBtnWrapper" style="display: {{ $hasAmenities ? 'block' : 'none' }};">
                          <button type="button" id="addAmenityBtn" class="btn btn-success btn-sm mb-3">
                            + Add Amenity
                          </button>
                        </div>

                        <!-- Amenity Form -->
                        <div id="amenityFormWrapper" style="display:none;">
                          <form id="amenityFormInline">
                            @csrf
                            ...
                            <div class="row g-2 align-items-center">
                              <div class="col-md-4">
                                <input type="text" id="amenityName" class="form-control" placeholder="Amenity name">
                              </div>
                              <div class="col-md-3">
                                <input type="number" id="amenityPrice" class="form-control" placeholder="Price">
                              </div>
                              <div class="col-md-3">
                                <select id="amenityStatus" class="form-control form-select">
                                  <option value="1">Active</option>
                                  <option value="0">Inactive</option>
                                </select>
                              </div>
                              <div class="col-md-2 d-flex gap-2">
                                <button type="button" id="saveAmenityBtn" class="btn btn-success btn-sm w-100">Save</button>
                                <button type="button" id="cancelAmenityBtn" class="btn btn-secondary btn-sm w-100">Cancel</button>
                              </div>
                            </div>
                          </form>
                        </div>
                        ...
                        <!-- Amenity Table -->
                        <div id="amenitiesWrapper" class="mt-4" style="display: {{ $hasAmenities ? 'block' : 'none' }};">
                            ...
                            <tbody>
                              @foreach($amenities as $i => $a)
                                <tr data-id="{{ $a->id }}">
                                  ...
                                  <td>
                                    <button type="button" class="btn btn-warning btn-sm editAmenityBtn"
                                      data-id="{{ $a->id }}"
                                      data-name="{{ $a->name }}"
                                      data-price="{{ $a->price }}"
                      ...
                      <div class="d-flex justify-content-between">
                        <button type="button" class="btn btn-primary btn-rounded prevButton">Back</button>
                        <button type="button" class="btn btn-secondary btn-rounded nextButton" data-next-tab="#profile-5">Next</button>
                      </div>
                    </div>
        ...
        if (formMode === 'edit' && propertyId) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('.utilitiesdata').addClass('done'); // ✅ Mark step as done
...
$(document).ready(function(){
    ...
    // === Step 1: Radio Button Logic ===
    $('input[name="is_billed"]').on('change', function(){
        if($(this).val() === 'yes'){
            $('#addAmenityBtnWrapper').slideDown();
            if ($('#amenitiesTable tbody tr').length > 0) {
                $('#amenitiesWrapper').slideDown();
            }
        } else {
            $('#addAmenityBtnWrapper, #amenityFormWrapper, #amenitiesWrapper').slideUp();
        }
    });

    // === Step 2: Add Amenity Click ===
    $('#addAmenityBtn').on('click', function(){
        // ✅ Always reset so form opens empty
        resetAmenityForm();

        $('#addAmenityBtnWrapper').hide();
        $('#amenityFormWrapper').slideDown();
    });

    // === Step 3: Cancel Form ===
    $('#cancelAmenityBtn').on('click', function(){
        resetAmenityForm();
        $('#amenityFormWrapper').slideUp();
        $('#addAmenityBtnWrapper').slideDown();
    });

    // === Step 4: Save Amenity (AJAX) ===
    $('#saveAmenityBtn').on('click', function(e){
        e.preventDefault();

        const id = $('#amenity_id').val();
        const name = $('#amenityName').val().trim();
        const price = $('#amenityPrice').val().trim();
        ...
        const url = id ? "{{ route('property-amenities-update') }}" : "{{ route('propertyamenities-store2') }}";

        $.ajax({
            url: url,
            type: 'POST',
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: { id, name, price, status, propertyid: propertyId },
            success: function(res){
                if(!res.success){
                    toastrs('warning', res.message, 'Warning');
                    return;
                }
                ...
                if (id) {
                    ...
                    row.find('td:nth-child(2)').text(d.name);
                    row.find('td:nth-child(3)').text(`$ ${d.price}`);
                    row.find('td:nth-child(4)').text(d.status == 1 ? 'Active' : 'Inactive');
                    row.find('.editAmenityBtn').data({
                        name: d.name,
                        price: d.price,
                        status: d.status
                    });
                    toastrs('success', 'Amenity updated successfully!', 'Success');
                } else {
                    // ✅ Add new row
                    ...
                    `);
                    toastrs('success', 'Amenity added successfully!', 'Success');
                }

                // ✅ Show table if it has rows
                if ($('#amenitiesTable tbody tr').length > 0) {
                    $('#amenitiesWrapper').slideDown();
                }

                // ✅ Add "done" class to the Amenities step indicator
                $('#profile-tab-4').find('.amenitiesdata').addClass('done');
            ...
            error: function(){
                toastrs('error', 'Failed to save amenity.', 'Error');
            }
        });
    });
    ...
    $(document).on('click', '.editAmenityBtn', function(){
        ...
        $('#amenityPrice').val(price);
        $('#amenityStatus').val(status);

        $('#addAmenityBtnWrapper').hide();
        $('#amenityFormWrapper').slideDown();
        $('#saveAmenityBtn').text('Update');
    });

    // === Helper: Reset Form ===
    function resetAmenityForm() {
        const form = document.getElementById('amenityFormInline');
        if (form) form.reset();
        $('#amenity_id').val('');
        $('#amenityName').val('');
        $('#amenityPrice').val('');
        $('#amenityStatus').val('1'); // default active
        $('#saveAmenityBtn').text('Save');
    }
});
</script>
