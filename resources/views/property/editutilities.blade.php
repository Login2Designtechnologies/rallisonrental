...
<div class="card bg-custom border p-25">
    <form action="{{url('addUtilities-update/'.$utilitiesdata->id.'/'.$propertyid)}}" method="post">
        @csrf
        <div class="row">
            <div class="col-lg-6">
                <div class="mb-3">
                    <label for="companyName" class="form-label">Company Name <span class="text-danger">*</span></label>
                    <input type="text" id="companyName" value="{{$utilitiesdata->name}}" name="name" class="form-control" placeholder="Enter Company Name" required>
                </div>
            </div>
            ...
            <div class="col-lg-6">
                <div class="mb-3">
                    ...
                    <select id="subcategorySelect" class="form-control form-select" name="sub_category" required>
                        <option value="0" {{ $utilitiesdata->sub_category == 0 ? 'selected' : '' }}>No</option>
                        <option value="1" {{ $utilitiesdata->sub_category == 1 ? 'selected' : '' }}>Yes</option>
                    </select>
                </div>
            </div>
            ...
            <div class="col-lg-12">
                <div id="subcategoryFields" class="mb-3">
                    ...
                    <input type="text" id="sub_category_name" value="{{$utilitiesdata->sub_category_name}}" name="sub_category_name" class="form-control">
                </div>
            </div>
            ...
            <div class="col-lg-6">
               <div class="mb-3">
                    ...
                    <select name="status" class="form-select">
                        <option value="1" {{ $utilitiesdata->status == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $utilitiesdata->status == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
            </div>
